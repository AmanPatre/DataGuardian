...
const FullReportView = ({ siteData = {}, onNavigate }) => {
  ...
  const groupedTrackers = (aiSummary?.trackerDetails || []).reduce((acc, tracker) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-4 md:p-6 space-y-6">
        ...
        <CollapsibleSection title="Detailed Tracker List" icon={<ListBulletIcon className="w-6 h-6 text-blue-600"/>} defaultOpen={false}>
            <div className="pt-4 space-y-4">
              {Object.keys(groupedTrackers).length > 0 ? (
                  ...
                  <div key={category}>
                    ...
                    <ul className="space-y-1">
                      {groupedTrackers[category].map(tracker => (
                        <li key={tracker.domain} className="text-sm text-gray-800 font-mono bg-gray-50 p-2 rounded flex justify-between items-center">
                          <span>{tracker.domain}</span>
                          <span className="text-gray-400 font-sans text-xs bg-white px-2 py-1 rounded-full">{tracker.company}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <p className="text-gray-600">No specific tracker domains were identified.</p>
              )}
            </div>
        </CollapsibleSection>
      </div>
